...
std::wstring ConvertString(const std::string& str) {
	...
	auto sizeNeeded = MultiByteToWideChar(CP_UTF8, 0,
		reinterpret_cast<const char*>(&str[0]), static_cast<int>(str.size()),
		NULL, 0);
	if (sizeNeeded == 0) {
		return std::wstring();
	}
...
void UploadTextureData(ID3D12Resource* texture, const DirectX::ScratchImage& mipImages) {
	...
	for (size_t mipLevel = 0; mipLevel < metadata.mipLevels; ++mipLevel) {
		...
		HRESULT hr = texture->WriteToSubresource(UINT(mipLevel),
			nullptr, img->pixels, UINT(img->rowPitch),
			UINT(img->slicePitch));
		assert(SUCCEEDED(hr));
	}
...
ID3D12Resource* CreateDepthStencilTextureResource(ID3D12Device* device, int32_t width, int32_t height) {
	...
	resouceDesc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
...
ID3D12Resource* CreateBufferResource(ID3D12Device* device, size_t sizelnBytes)
{
	...
	vertexResourceDesc.Width = sizelnBytes;
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	*/
	...
	ID3D12DescriptorHeap* rtvDescriptoHeap = CreateDescriptorHeap(device, D3D12_DESCRIPTOR_HEAP_TYPE_RTV, 2, false);
